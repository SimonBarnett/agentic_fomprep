/**
 * Local execute MCP: list_instances / compile_shell / get_last_result / get_skill.
...
const skillMd = path.join(pluginRoot, 'skills', 'priority-shell-compile', 'SKILL.md');
const runner = path.join(pluginRoot, 'scripts', 'Compile-Shell.ps1');
...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        revision: { type: 'string', description: 'Version Revision NN' },
      },
  ...
  },
  {
    ...
    description: 'Last compile_shell result for an instance.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'get_skill',
    description: 'Local SKILL.md for priority-shell-compile.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
  },
];
...
function runCompile({ instance_id, revision }) {
...
async function callTool(name, args) {
  ...
  if (name === 'compile_shell') {
    if (!a.instance_id || a.revision === undefined || a.revision === null || a.revision === '') {
      return text('instance_id and revision required', true);
    }
    return runCompile({ instance_id: a.instance_id, revision: a.revision });
  }
  if (name === 'get_last_result') {
    ...
    const last = path.join(workRoot(inst), 'last-compile.json');
  ...
  if (method === 'initialize') {
    return {
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'priority-shell-compile-local', version: '0.1.0' },
        instructions: 'Use list_instances then compile_shell. Separate from install_shell. Catalog at https://mcp-priority.ntsa.uk/mcp is grab-only.',
      },
    };
  }
